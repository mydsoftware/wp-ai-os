...
class WP_AI_OS_API_Controller {
	...
	public function register_routes(): void {
		...
		register_rest_route( self::NAMESPACE, '/content/assist', array( 'methods' => WP_REST_Server::CREATABLE, 'callback' => array( $this, 'content_assist' ), 'permission_callback' => array( $this, 'permissions' ), 'args' => array( 'post_id' => array( 'required' => true, 'type' => 'integer', 'minimum' => 1 ), 'task' => array( 'required' => false, 'type' => 'string', 'default' => 'optimize' ) ) ) );
	}
	...
	public function settings_update( WP_REST_Request $request ): WP_REST_Response {
		...
		if ( ! is_array( $settings ) ) { return new WP_REST_Response( array( 'code' => 'invalid_settings' ), 400 ); }
		if ( isset( $settings['ai_api_key'] ) && '********' === $settings['ai_api_key'] ) { unset( $settings['ai_api_key'] ); }
		WP_AI_OS_Settings::update( $settings );
		return $this->settings_get();
	}

	public function ai_test(): WP_REST_Response {
		$result = ( new WP_AI_OS_OpenAI_Compatible_Provider() )->generate( 'You are a concise WordPress AI assistant.', 'Reply with exactly: WP AI OS connection OK', array( 'temperature' => 0 ) );
		if ( is_wp_error( $result ) ) { return new WP_REST_Response( array( 'success' => false, 'code' => $result->get_error_code(), 'message' => $result->get_error_message() ), 400 ); }
	...
	public function content_analyze( WP_REST_Request $request ): WP_REST_Response {
		return new WP_REST_Response( ( new WP_AI_OS_Content_Analyzer() )->analyze( (string) $request->get_param( 'content' ), (string) $request->get_param( 'title' ) ), 200 );
	}

	public function content_assist( WP_REST_Request $request ): WP_REST_Response {
		$result = ( new WP_AI_OS_Content_Assistant() )->generate_for_post( absint( $request->get_param( 'post_id' ) ), sanitize_key( (string) $request->get_param( 'task' ) ) );
		if ( is_wp_error( $result ) ) { return new WP_REST_Response( array( 'success' => false, 'code' => $result->get_error_code(), 'message' => $result->get_error_message() ), 400 ); }
