...
class WP_AI_OS_API_Controller {
	...
	public function register_routes(): void {
		$routes = array(
			'/status' => array( 'methods' => WP_REST_Server::READABLE, 'callback' => array( $this, 'status' ) ),
			'/readiness/scan' => array( 'methods' => WP_REST_Server::CREATABLE, 'callback' => array( $this, 'scan' ) ),
			'/ai/test' => array( 'methods' => WP_REST_Server::CREATABLE, 'callback' => array( $this, 'ai_test' ) ),
			'/content/analyze' => array( 'methods' => WP_REST_Server::CREATABLE, 'callback' => array( $this, 'content_analyze' ) ),
			'/content/assist' => array( 'methods' => WP_REST_Server::CREATABLE, 'callback' => array( $this, 'content_assist' ) ),
			'/knowledge/index' => array( 'methods' => WP_REST_Server::CREATABLE, 'callback' => array( $this, 'knowledge_index' ) ),
			'/knowledge/search' => array( 'methods' => WP_REST_Server::READABLE, 'callback' => array( $this, 'knowledge_search' ) ),
			'/knowledge/ask' => array( 'methods' => WP_REST_Server::CREATABLE, 'callback' => array( $this, 'knowledge_ask' ) ),
			'/agent/tasks' => array( 'methods' => WP_REST_Server::READABLE, 'callback' => array( $this, 'agent_tasks' ) ),
			'/agent/run' => array( 'methods' => WP_REST_Server::CREATABLE, 'callback' => array( $this, 'agent_run' ) ),
		);
		foreach ( $routes as $path => $args ) {
			$args['permission_callback'] = array( $this, 'permissions' );
			register_rest_route( self::NAMESPACE, $path, $args );
		}
		register_rest_route( self::NAMESPACE, '/settings', array( array( 'methods' => WP_REST_Server::READABLE, 'callback' => array( $this, 'settings_get' ), 'permission_callback' => array( $this, 'permissions' ) ), array( 'methods' => WP_REST_Server::EDITABLE, 'callback' => array( $this, 'settings_update' ), 'permission_callback' => array( $this, 'permissions' ) ) ) );
	}
	...
	public function knowledge_index(): WP_REST_Response { return new WP_REST_Response( array( 'indexed' => ( new WP_AI_OS_Knowledge_Base() )->index_all() ), 200 ); }
	...
	public function agent_tasks(): WP_REST_Response { return new WP_REST_Response( array( 'tasks' => ( new WP_AI_OS_Agent_Runner() )->tasks() ), 200 ); }
	public function agent_run( WP_REST_Request $request ): WP_REST_Response { $result = ( new WP_AI_OS_Agent_Runner() )->run( sanitize_key( (string) $request->get_param( 'task' ) ), is_array( $request->get_param( 'args' ) ) ? $request->get_param( 'args' ) : array() ); if ( is_wp_error( $result ) ) { return new WP_REST_Response( array( 'success' => false, 'code' => $result->get_error_code(), 'message' => $result->get_error_message() ), 400 ); } return new WP_REST_Response( array( 'success' => true, 'data' => $result ), 200 ); }
}
